    ...
    def prepare_inputs_for_generation(
        ...
        **kwargs,
        ...
        if past_key_values is not None:
            ...
            # ---- 手动为新 token 拼接 ih_ids=ih_size-1 ----
            # if ih_ids is not None:
            #     # 假设已经完成第一步 ih_ids 的赋值 (指令/输入部分)，此时在多步生成中，
            #     batch_size = input_ids.shape[0]
            #     # 注意要将 ih_ids 放在同一个 device 上
            #     ih_ids = ih_ids.new_full(
            #         (batch_size, 1), 
            #         fill_value=(self.ih_size - 1)
            #     )

            #     # 如果 new_len < old_len，说明出现了截断或别的操作，你可以根据需要决定如何处理
            # print('====== ih_ids:\n', ih_ids, ih_ids.shape)
        ...
        # 6.5. 把 ih_ids 放进 model_inputs
        if ih_ids is not None:
            # 同样可以做个 clone 保持一致的 stride
            ih_ids = ih_ids.clone(memory_format=torch.contiguous_format)
            model_inputs["ih_ids"] = ih_ids

        # 显示打印一下，方便调试
        # print("============")
        # print('model_inputs:', {k: v.shape if hasattr(v, 'shape') else v for k, v in model_inputs.items()})
    ...
    def _sample(
        ...
        **model_kwargs,
        ...
        while self._has_unfinished_sequences(
            ...
            # --- new code here: 同步为 ih_ids 拼接 “self.ih_size - 1” ---
            if "ih_ids" in model_kwargs and model_kwargs["ih_ids"] is not None:
                ...
                ih_ids_new = torch.full(
                    ...
                    fill_value=(self.ih_size - 1),  # 你想表示“回复段”的id
        ...
        if return_dict_in_generate:
            if self.config.is_encoder_decoder:
                ...
                # ========== 添加 Qformer_past_key_values, 一般不执行此逻辑, 而执行最下面的 return input_ids ==========
