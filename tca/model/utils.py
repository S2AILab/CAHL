...
class ConstantLengthDatasetForInstructionHierarchy(ConstantLengthDataset):
    """
    When dataset is pretokenized, 
        ConstantLengthDataset only take `input_ids` using a `formatting_func` which returns `x['input_ids']`
        ConstantLengthDatasetForInstructionHierarchy will take `input_ids` and `ih_ids` using a `formatting_func` which returns `x` directly
    ...
    def __iter__(self):
        # ConstantLengthDatasetForInstructionHierarchy takes a `formatting_func` -> {'input_ids': ..., 'ih_ids': ...}
    ...
    def _tokenize(self, buffer):
        ...
        for i in range(len(buffer)):
            ...
            ih_id = torch.zeros_like(input_ids[i], dtype=torch.long)
            ih_id[inst_start_idx: input_start_idx] = 1 if self.ih_size == 4 else 0
            ih_id[input_start_idx: output_start_idx] = 2 if self.ih_size == 4 else 1
            ih_id[output_start_idx: ] = 3 if self.ih_size == 4 else 2
            buffer[i] = {
...
class GenerationMixinForIH(GenerationMixin):
    ...
    def prepare_inputs_for_generation(
        ...
        **kwargs,
        ...
        # 7. put ih_ids into model_inputs
        if ih_ids is not None:
...
class GenerationMixinForQformer(GenerationMixin):
    ...
    def generate(
        ...
        **kwargs,
        ...
        # use the code below when transformer>=4.49.0
